...
public class List implements Control {
	private JTextField text;
	private int width;
	private int height;

	public static List create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new List(text1, width, height);
	}

	public List(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					List.this.text.setText(List.this.text.getText().replaceAll("\\D", ""));
				}
