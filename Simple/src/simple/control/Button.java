...
public class Button implements Control {
	...
	public static Button create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new Button(text1, width, height);
	}

	public Button(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					Button.this.text.setText(Button.this.text.getText().replaceAll("\\D", ""));
				}
