...
public class Area implements Control {
	private JTextField text;
	private int width;
	private int height;

	public static Area create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new Area(text1, width, height);
	}

	public Area(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					Area.this.text.setText(Area.this.text.getText().replaceAll("\\D", ""));
				}
