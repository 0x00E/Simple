...
public class Radio implements Control {
	...
	public static Radio create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new Radio(text1, width, height);
	}

	public Radio(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					Radio.this.text.setText(Radio.this.text.getText().replaceAll("\\D", ""));
				}
