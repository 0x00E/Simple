...
public class Label implements Control {
	...
	public static Label create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new Label(text1, width, height);
	}

	public Label(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					Label.this.text.setText(Label.this.text.getText().replaceAll("\\D", ""));
				}
