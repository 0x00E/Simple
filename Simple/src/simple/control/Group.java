...
public class Group implements Control {
	...
	public static Group create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new Group(text1, width, height);
	}

	public Group(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					Group.this.text.setText(Group.this.text.getText().replaceAll("\\D", ""));
				}
