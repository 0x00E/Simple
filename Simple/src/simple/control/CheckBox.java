...
public class CheckBox implements Control {
	...
	public static CheckBox create(String text, int width, int height) {
		JTextField text1 = new JTextField(text);
		return new CheckBox(text1, width, height);
	}

	public CheckBox(JTextField text, int width, int height) {
		this.text = text;
		...
		if (format == Format.Integer) {
			this.text.addKeyListener(new KeyListener() {
				public void keyTyped(KeyEvent e) {
					...
					CheckBox.this.text.setText(CheckBox.this.text.getText().replaceAll("\\D", ""));
				}
